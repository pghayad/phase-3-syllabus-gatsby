...
<YoutubeEmbed link="https://www.youtube.com/embed/videoseries?list=PLc6AmvC5ZybzSIU0jajn7JT5OQmE8xDQY " />

Make sure to watch the entire playlist! You can also access it directly on Youtube: https://www.youtube.com/watch?v=_t__xI_luXU&list=PLc6AmvC5ZybzSIU0jajn7JT5OQmE8xDQY

# Code

The starter code and completed code for this lecture can be accessed here: https://github.com/ihollander/js-lectures-async-javascript-and-fetch
...
In the snippet above, we're sending an HTTP `GET` request to our url using `fetch`. Note that if we don't specify the HTTP verb (`GET`, `POST`, `PUT/PATCH`, `DELETE`) `fetch` will default to a `GET` request. So this will work the same:
